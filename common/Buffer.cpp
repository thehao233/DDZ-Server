...
int Buffer::appendDataHead(int headLen) {

    // 将数据头转化为大端存储 用于网络传输
    headLen = htonl(headLen);
    int ret = appendString(reinterpret_cast<char*>(&headLen), sizeof(int));
    return ret;
}

int Buffer::appendDataPackage(const string data) {
//    std::cout << "写入前 readPos:" << m_readPos << " writePos:" << m_writePos << std::endl;
    // 写入数据长度
    appendDataHead(data.size());
    // 写入数据
    appendString(data.c_str(), data.size());
//    std::cout << "写入后 readPos:" << m_readPos << " writePos:" << m_writePos << std::endl;
    return 0;
}
